...
// PLASTIBOT MENDEL
// X-end-motor.
// Added room for 32T pulley. Added screw for Z endstop fine adjustment.
...
m3_screw_dia = 3; //was 3
m3_screw_head_dia = 6;
m3_screw_head_height = 3;
m3_nut_wrench_size = 5.5; //was 5.5
m3_nut_height = 2.4;

m8_screw_dia = 8; //was 8
m8_washer_dia = 16;
m8_washer_height = 1.5;

idler_dia = 22;		// when using pure 608 bearing without belt guide: 22mm - was 25
...
leadscrew_dia = 7; // was 8
leadscrew_nut_wrench_size = 13.4+0.2;		// plus some extra clearance - was 13
...
difference() {
	union() {
		...
		//}
		translate([-38,18,0]) difference() {
			cube([10,15,10]);
			translate([5,5,0])
				polyhole(m3_screw_dia+2*clearance,xend_body_height+10);
			translate([5,5,0])
				nut_trap(m3_nut_wrench_size,m3_nut_height+1.6);
		}
		translate([-38,17,m3_nut_height-0.4])
			cube([10,15,0.4]);	
	}
	translate([-17.5, 50, 31.75]) rotate([90, 0, 0]) {
		cylinder(r=15, h=20);
		translate([0, -13, 0]) cube([100, 26, 20]);
